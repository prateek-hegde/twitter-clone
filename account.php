<?php
	...
	if(isset($_POST['submit'])){
		$username = $getUser->checkInput($_POST['username']);
		$email 	  = $getUser->checkInput($_POST['email']);
		$error    = array();

		if(!empty($username) && !empty($email)){
			if($user->username != $username && $getUser->checkUsername($username) === true){
				$error['username'] = "The username is not available"; 
			} else if(preg_match("/[^a-zA-Z0-9\!]", $username)) {
				$error['username'] = "Only Alphanumeric charecters are allowed"; 
			} else if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
				$error['email'] = "Invalid email format";
			} else if($user->email != $email && $getUser->checkEmail($email) === true){
				$error['email'] = "This email is already in use";
			} else {
				$getUser->update('users', $user_id, array('username' => $username, 'email' => $email));
