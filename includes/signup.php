	...
	if(isset($_GET['step']) && empty($_GET['step']) === false){
		if(isset($_POST['next'])){
			...
			if(strlen($username) > 20){
				$error = 'UserName must be less than 20 charecter';
			} else if($getUser->checkUsername($username) === true){
				$error = 'Sorry! this UserName is already taken..!';
			} else {
				$getUser->update('users', $user_id, array('username' => $username));
  				...
  				?>
					...
					<div class='lets-wrapper'>
							<div class='step-letsgo'>
								<h2>We're glad you're here, <?php if(isset($user->username)) { echo '@'.$user->screenName; }?></h2>
