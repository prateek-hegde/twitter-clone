<?php
	...
	if(isset($_POST['hashtag'])){
		$hashtag = $getUser->checkInput($_POST['hashtag']);
		$mention = $getUser->checkInput($_POST['hashtag']);

		if(substr($hashtag, 0, 1) === '#'){
			$trend = str_replace('#', '', $hashtag);
			$trends = $getTweet->getTrendByHash($trend);
		...
		if(substr($mention, 0, 1) === '@'){
			...
			$mentions = $getTweet->getMention($mention);
