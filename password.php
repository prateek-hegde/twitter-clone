<?php
	...
	if(isset($_POST['submit'])){
		$currentPwd  = $getUser->checkInput($_POST['currentPwd']);
		$newPassword = $getUser->checkInput($_POST['newPassword']);
		$rePassword  = $getUser->checkInput($_POST['rePassword']);
		$error       = array();

		if(!empty($currentPwd) && !empty($newPassword) && !empty($rePassword)){
			if($user->password != $getUser->securedPassword($currentPwd)){
				$error['currentPwd'] = "Invalid current Password";
			} else if($newPassword != $rePassword){
				$error['rePassword'] = "Passwords do not match";
			} else if(strlen($rePassword) < 6){
				$error['newPassword'] = "Password is too short";
			} else {
				$rePassword = $getUser->securedPassword($rePassword);
				$getUser->update('users', $user_id, array('password' => $rePassword));
				header('Location: '.BASE_URL.'settings/password');
				$error['fields'] = "Password chanegd succesfully";
			}
		} else {
